...
describe("FileExporter", () => {
  context("generateFileName", () => {
    ...
    it(".jsonの拡張子がない場合、.jsonが末尾につく", () => {
      chai.expect(generateFileName("original")).eql("original.json");
    });

    it("引数未指定の場合、デフォルトのファイル名になる", () => {
      chai.expect(generateFileName()).eql(DEFAULT_FILE_NAME);
    });
    ...
    it("特殊文字はエスケープされる", () => {
      chai.expect(generateFileName('/?<>\\:*|".json')).eql("_________.json");
    });
